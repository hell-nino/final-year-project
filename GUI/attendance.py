...
def take_attendence():
    attendence_root.withdraw()  # Hide the main window
    take_atten = tk.Tk()
    ...
    back_button = tk.Button(take_atten, text="Back", command=take_atten.destroy)
    ...
    update_button = tk.Button(take_atten, text="Get Current Date", command=lambda: get_current_date(date_entry))
    update_button.pack()
    
    # Create a Start button

    start_button = tk.Button(take_atten, text="Start", command=start_attendance)
...
b1 = tk.Button(center_frame, text="Take Attendance", fg="red", font=("comicsense", 19, "bold"), borderwidth=3, relief=tk.SUNKEN, command=take_attendence)
b1.pack(pady=10)  # Add some vertical padding between the buttons

# Create a button for enrolling
b2 = tk.Button(center_frame, text="Enroll", fg="red", font=("comicsense", 19, "bold"), borderwidth=3, relief=tk.SUNKEN)
b2.pack(pady=10)
